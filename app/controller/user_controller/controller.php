...
function iniciarSesion($usuario, $contrasena)
{
    ...
    $conn->query("SET @salida3 = '';");
    // Preparar y ejecutar el procedimiento almacenado con variables de usuario
    $stmt = $conn->prepare("CALL 01_hotel_dba_sp_login(?, ?, @salida1, @salida2,@salida3)");
    if ($stmt) {
        ...
        $result = $conn->query("SELECT @salida1 AS rol, @salida2 AS validacion, @salida2 AS idUsuario");
        if ($result) {
...
// $resultado = registroUsuario(11, 'jfray', '', '',  '', 'contrasena', 'naelfray@gmail.com', '');
